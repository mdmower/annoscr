...
```sh
# Debian / Ubuntu
sudo apt install ./annoscr_*_all.deb

# Fedora / openSUSE
sudo dnf install ./annoscr-*.noarch.rpm

# Arch (import the signing key first; see Verifying downloads)
sudo pacman -U ./annoscr-*-any.pkg.tar.zst
```
...
```sh
gpg --verify annoscr_1.0.3_all.deb.asc annoscr_1.0.3_all.deb

gpg --verify annoscr-1.0.3-1-any.pkg.tar.zst.sig annoscr-1.0.3-1-any.pkg.tar.zst
```
...
# install
sudo pacman -U ./annoscr-1.0.3-1-any.pkg.tar.zst
```
