...
- **Styling**
  ...
  - Stroke width, line style (solid / dashed / dotted), an arrow's head and tail (each none, wings, or filled), and rounded rectangle corners.
...
```sh
# Debian / Ubuntu
sudo apt install ./annoscr_2.2.0_all.deb

# Fedora / openSUSE
sudo dnf install ./annoscr-2.2.0-1.fc43.noarch.rpm

# Arch (import the signing key first; see Verifying downloads)
sudo pacman -U ./annoscr-2.2.0-1-any.pkg.tar.zst
```
...
```sh
gpg --verify annoscr_2.2.0_all.deb.asc annoscr_2.2.0_all.deb

gpg --verify annoscr-2.2.0-1-any.pkg.tar.zst.sig annoscr-2.2.0-1-any.pkg.tar.zst
```
